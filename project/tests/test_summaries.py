...
def test_create_summaries_invalid_url_scheme(test_app):
...
def test_update_summary_invalid_url_scheme(test_app_with_db):
    response = test_app_with_db.post("/summaries/", json={"url": "https://foo.bar"})
    summary_id = response.json()["id"]
...
def test_remove_summary_invalid_id(test_app_with_db):
...
def test_read_summary_valid_id(test_app_with_db):
